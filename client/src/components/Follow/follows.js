...
function Follows(props) {
        ...
        axios.get(axiosUrlGetFollows+url.slice(last_slash+1))
            .then(res=>{
                if(res.data.follow.length > 0){
                    setBFollows(1)
                    let i = index_follow
                    for(i;i<res.data.follow.length;i++){
                        create_follow(res.data.follow[i])
                    }
                    setIndexFollow(i)
                }
                else {
    ...
    return (
        ...
        <Navbar/> 
        <p id='p_follow_msg'>Abonnements :</p>
        <div id='follows-section'>{b_follows === 1 ? <p/>:<p id='no_follows'>{username} est abonné à aucun compte</p>}</div>
